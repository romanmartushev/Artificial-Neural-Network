...
def main():
    ...
    weightsHI = [[uniform(-1,1),uniform(-1,1)],[uniform(-1,1),uniform(-1,1)]]
    # Array for each output node with as many values as hidden nodes + bias Nodes
    weightsOH = [[uniform(-1,1),uniform(-1,1),uniform(-1,1)]]
    ...
    while count != 10000:
        ...
        for i in range(0,len(nodesO)):
            outputs.append((nodesO[i].getOutput() * (max - min) + min)*2)
            errors.append(pow((outputList[j] * (max - min) + min)*2 - (nodesO[i].getOutput() * (max - min) + min)*2,2))
            myData =[[outputList[j] * (max - min) + min,(nodesO[i].getOutput() * (max - min) + min)*2,(outputList[j] * (max - min) + min)*2]]
            myFile = myFile = open('outputs.csv','a')
            with myFile:
                writer = csv.writer(myFile)
                writer.writerows(myData)
            print("beginning input: " + str(outputList[j] * (max - min) + min) + " current result: " + str((nodesO[i].getOutput() * (max - min) + min)*2) + " actual: " + str((outputList[j] * (max - min) + min)*2) + " epoch: " + str(count))
        ...
        for i in range(0,len(nodesO)):
            nodesO[i].setDelta(outputList[j],max,min)
    ...
    for i in range(0,len(outputs)):
        myData = [[outputs[i],(outputList[i] * (max - min) + min)*2]]
        myFile = myFile = open('final.csv','a')
        with myFile:
            writer = csv.writer(myFile)
            writer.writerows(myData)
